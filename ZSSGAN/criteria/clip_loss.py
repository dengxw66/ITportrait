...
import os
import sys
sys.path.append("./ZSSGAN/criteria")
...
class Middle_directionLoss(torch.nn.Module):

    def __init__(self, loss_type='cosine'):
        super(Middle_directionLoss, self).__init__()
    ...
    # @snoop()
    def forward(self,edit_direction1, edit_direction2,target_direction,weight_text=1.0,weight_img=1.0):
        
        if self.loss_type == "cosine":
            c = weight_text*target_direction + weight_img*edit_direction2 
            return 1. - self.loss_func(edit_direction1, c)
        
        return self.loss_func(edit_direction1,target_direction)


class CLIPLoss_middle(torch.nn.Module):
    ...
    def __init__(self, device, lambda_direction=1., lambda_patch=0., lambda_global=0., lambda_manifold=0., lambda_texture=0., patch_loss_type='mae', direction_loss_type='cosine', clip_model='ViT-B/32'):
        super(CLIPLoss_middle, self).__init__()
        ...
        self.direction_middle_Loss =Middle_directionLoss(direction_loss_type)
    ...
    def clip_angle_loss(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str) -> torch.Tensor:
        ...
        return self.angle_loss(cos_img_angle, cos_text_angle) # torch.nn.L1Loss()
    ...
    def clip_directional_img_loss(self, src_img1: torch.Tensor, src_img2: torch.Tensor, target_img1: torch.Tensor, target_img2: torch.Tensor) -> torch.Tensor:

        src_encoding1    = self.get_image_features(src_img1)
        target_encoding1 = self.get_image_features(target_img1)
        edit_direction1 = (target_encoding1 - src_encoding1)
        if edit_direction1.sum() == 0:
            target_encoding1 = self.get_image_features(target_img1 + 1e-6)
            edit_direction1 = (target_encoding1 - src_encoding1)
        edit_direction1 /= (edit_direction1.clone().norm(dim=-1, keepdim=True))

        src_encoding2    = self.get_image_features(src_img2)
        target_encoding2 = self.get_image_features(target_img2)
        edit_direction2 = (target_encoding2 - src_encoding2)
        if edit_direction2.sum() == 0:
            target_encoding2 = self.get_image_features(target_img2 + 1e-6)
            edit_direction2 = (target_encoding2 - src_encoding2)
        edit_direction2 /= (edit_direction2.clone().norm(dim=-1, keepdim=True))
        
        return self.direction_loss(edit_direction1, edit_direction2).mean()
    ...
    def clip_img_middle_directional_loss(self, epoch: int, src_img1: torch.Tensor, src_img2: torch.Tensor, target_img1: torch.Tensor, target_img2: torch.Tensor,source_class: str,target_class: str) -> torch.Tensor:

        source_features = self.get_text_features(source_class)
        target_features = self.get_text_features(target_class)
        text_direction = (target_features - source_features).mean(axis=0, keepdim=True)
        text_direction_length= text_direction.norm(dim=-1, keepdim=True) 
        text_direction /= text_direction.norm(dim=-1, keepdim=True) 
        ...
        target_encoding1 = self.get_image_features(target_img1)
        edit_direction1 = (target_encoding1 - src_encoding1)
        edit_direction1_length = (edit_direction1.clone().norm(dim=-1, keepdim=True)) 
        edit_direction1 /= (edit_direction1.clone().norm(dim=-1, keepdim=True)) 

        src_encoding2    = self.get_image_features(src_img2)
        target_encoding2 = self.get_image_features(target_img2)
        edit_direction2 = (target_encoding2 - src_encoding2)
        edit_direction2_length = (edit_direction2.clone().norm(dim=-1, keepdim=True)) 
        edit_direction2 /= (edit_direction2.clone().norm(dim=-1, keepdim=True)) 

        train_length=(target_encoding1-target_encoding2)
        train_length = train_length.clone().norm(dim=-1, keepdim=True) 

        # 阈值
        num_range=random.randint(1,100) 
        threshold=35   
        if edit_direction2_length.mean()<0.65 or num_range <threshold: 
            # weight_img=0    
            # weight_text=1.0   
            weight_img=0.5    
            weight_text=0.5  
            cos_func=torch.nn.CosineSimilarity()
            angle = cos_func(edit_direction1,self.target_direction).mean() 
            print("#-------------- text direction --------------#")
            return self.direction_middle_Loss(edit_direction1, edit_direction2,self.target_direction,weight_text,weight_img).mean() 
        else: 
            cos_func=torch.nn.CosineSimilarity()
            angle = cos_func(edit_direction2,self.target_direction).mean() 
            # weight_img=1.0    
            # weight_text=0   
            weight_img=0.5    
            weight_text=0.5   
    ...
    def clip_text_directional_loss(self, epoch: int, src_img1: torch.Tensor, src_img2: torch.Tensor, target_img1: torch.Tensor, target_img2: torch.Tensor,source_class: str,target_class: str) -> torch.Tensor:
    ...
    def forward(self,fix_target_img,epoch,args, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor, target_class: str, texture_image: torch.Tensor = None):
        # aug_multi_view_imgs_train_fix,epoch,args,aug_multi_view_imgs_frozen, self.source_class, aug_multi_view_imgs_train, self.target_class

        clip_loss = 0.0
        batch=args.batch

        if args.type=="fusion":
            clip_loss +=  self.clip_img_middle_directional_loss(epoch,src_img, src_img, target_img, fix_target_img, source_class,target_class)
