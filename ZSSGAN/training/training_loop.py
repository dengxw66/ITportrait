...
import torch
import dnnlib
from torch_utils import misc # 这里也不知道是什么，里面有很多的小函数，后续都大量使用到
from torch_utils import training_stats # 一个优点特殊的东西？，后续都大量使用到
from torch_utils.ops import conv2d_gradfix # 这个是为了使用stylegan2
from torch_utils.ops import grid_sample_gradfix # 这个是为了使用stylegan2

import legacy
from metrics import metric_main # 一样的加载metric
from camera_utils import LookAtPoseSampler # 没有用到
from training.crosssection_utils import sample_cross_section # 没有用到

#----------------------------------------------------------------------------
# 这个函数用于导入Exporting sample images，后面的trainingloop只用到了一次
...
#----------------------------------------------------------------------------
# 这里应该就是保存图片，后续trainingloop有多个地方使用
...
def training_loop(
    ...
    if flag:# Resume from existing pickle.
    ...
    if rank == 0:# Print network summary tables.
        z = torch.empty([batch_gpu, G.z_dim], device=device)
        c = torch.empty([batch_gpu, G.c_dim], device=device)
        img = misc.print_module_summary(G, [z, c])
        misc.print_module_summary(D, [img, c])

    if flag:# Setup augmentation.
        if rank == 0:
    ...
    if flag:# Distribute across GPUs.
        if rank == 0:
    ...
    if flag:# Train.
        ...
        while True:
            ...
            if flag:# Print status line, accumulating the same information in training_stats.
